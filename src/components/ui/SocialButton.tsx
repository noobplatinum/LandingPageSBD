...
const SocialButtons: React.FC<SocialButtonsProps> = ({ className, onButtonClick }) => {
  const { darkMode } = useTheme();
  
  // Keep your existing click handler
  const handleClick = (platform: string) => {
    console.log(`Clicked ${platform} button`);
    if (onButtonClick) onButtonClick(platform);
    
    // Add actual URL redirects
  ...
  return (
    <div className={`main ${className || ''}`} style={{ fontSize: '1rem' }}>
      <div className="up">
        <button 
          className={`card1 ${darkMode ? 'dark-mode' : ''}`} 
        ...
        >
          <svg 
            ...
            fillRule="nonzero" 
            className="instagram"
            style={{ position: 'absolute', top: '1.5em', left: '1.2em' }}
          >
        ...
        <button 
          className={`card2 ${darkMode ? 'dark-mode' : ''}`} 
        ...
        >
          <svg 
            ...
            height="30px" 
            className="twitter"
            style={{ position: 'absolute', top: '1.5em', left: '-0.9em' }}
          >
      ...
      <div className="down">
        <button 
          className={`card3 ${darkMode ? 'dark-mode' : ''}`} 
        ...
        >
          <svg 
            ...
            width="30px" 
            height="30px" 
            className={`github ${darkMode ? 'dark-mode-icon' : ''}`}
            style={{ position: 'absolute', top: '-0.6em', left: '1.2em' }}
          >    
        ...
        <button 
          className={`card4 ${darkMode ? 'dark-mode' : ''}`} 
        ...
        >
          <svg 
            ...
            xmlns="http://www.w3.org/2000/svg" 
            className="discord"
            style={{ position: 'absolute', top: '-0.9em', left: '-1.2em' }}
          >
