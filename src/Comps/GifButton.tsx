...
const GifButton: React.FC<GifButtonProps> = ({ isExpanded, handleGifClick }) => {
    ...
    return (
        <div className={`absolute right-0 top-0 -mt-3 transform transition-transform duration-1000 ${
          slidePosition ? 'translate-y-32' : 'translate-y-0'
        }`}>
          <div className="relative">
            <div className={`flex items-center gap-4 text-white p-2 transition-opacity duration-500 ${
              showExtension ? 'opacity-100' : 'opacity-0'
            } absolute`} style={{ right: '100%', marginRight: '15px' }}>
